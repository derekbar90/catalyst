...
export const PermissionsMiddleware = (options: PermissionsMiddlewareOptions) => ({
	...
	localAction(next: ActionHandler, action: ActionParams & { name: string }) {
		...
		const isEntityOwner = async (ctx: Context): Promise<boolean> => {
			if (typeof ctx.service.isEntityOwner === 'function') {
				...
				throw new Errors.MoleculerServerError("You are missing the isEntityOwner action on this service", 500, "ERR_ACCESS_CTRL_OWNER_HANDLER_MISSING", { action: action.name });
			}
		}
		...
		if (action.permissions) {
			...
			const permissions: PermissionConfig[] = Array.isArray(action.permissions) ? action.permissions : [action.permissions];
			...
			return async (ctx: Context<{}, { roles: string[], user: { [key: string]: any } } & ActionParams>) => {
				const user = ctx.meta.user;
				if (
					user == undefined ||
					user == null
				) {
					throw new Errors.MoleculerClientError("You have not provided the appropriate access token for this resource.", 401, "ERR_HAS_NO_ACCESS", { action: action.name });
				}
				...
				if (roles) {
					...
					if (res !== true) {
						...
						if (res !== true) {
							throw new Errors.MoleculerClientError("This account does not have enough permissions for this action.", 401, "ERR_HAS_NO_PERMISSIONS", { action: action.name });
						}
					}
				}
